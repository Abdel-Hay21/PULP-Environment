...
int main()
{
  ...
  {
    Write_Data(DATA_SIZE_PER_EQUALIZER, &(data_in[2*EQUALIZER_Size*i]), (uint32_t *) Data_location[i]);
  }
  ...
  //   █      ▄▄█▄▄  █   ██ ▄▄█▄▄  ▀▄▄▄█▀ █    █        █   █   █▄▄█  █    ▀ █   ▀▄ ▄▄█▄▄  █   ██  ▀▄▄▄▀
     ...
     for(int i=0 ; i<N_LOC * N_FFT ; i++)
     {
      Check_correctness(i, DATA_SIZE_PER_EQUALIZER, &(data_out[2*EQUALIZER_Size*i]) , (uint32_t  ) Data_location[i]);
     }
  return 0;
}
