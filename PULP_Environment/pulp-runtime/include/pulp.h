...
extern volatile void  *cluster_entry[ARCHI_NB_CLUSTER];
extern char           *cluster_stacks[ARCHI_NB_CLUSTER];

/* entry is unprototyped int(*)() so it accepts any function pointer
 * (void(*)(void), int(*)(int,char**), int(*)() etc.) without error.
 * cluster_entry_stub casts it to void(*)(void) internally.           */
...
static inline int cluster_wait_all(void)    { return 0; }
