...
module mac_fsm (
  ...
  begin : main_fsm_comb
    ...
    // a stream
    ctrl_streamer_o.a_source_ctrl.addressgen_ctrl.trans_size  = ctrl_i.len; // عدد العناصر اللي هنقراهم
    ctrl_streamer_o.a_source_ctrl.addressgen_ctrl.line_stride = '0; 
    ctrl_streamer_o.a_source_ctrl.addressgen_ctrl.line_length = ctrl_i.len; //no 2D processing so the vector is line (trans_size)
