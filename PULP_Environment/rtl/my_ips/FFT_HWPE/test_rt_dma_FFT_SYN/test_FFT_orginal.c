...
uint32_t data_out_location_l2 [ TASK_NUM ][FFT_Size * Num_Frames] __attribute__((section(".l2_data"), aligned(4)));
                ...
                for(int j=0 ; j<4 ; j++)
                {
                 // Data transfer from  L1 --> L2 
                    dma_out = plp_dma_memcpy(
                                             (uint32_t  ) (data_out_location_l2[i-2] +  1 * j * (4096 / 4))        , // Data Distination    (       L2         )
...
int main()
{
  ...
  printf("\ntask 1 address = 0x%08x\n", data_out_location_l2[0]);
  printf("\ntask 2 address = 0x%08x\n", data_out_location_l2[1]);
  printf("\ntask 3 address = 0x%08x\n", data_out_location_l2[2]);
  printf("\ntask 4 address = 0x%08x\n", data_out_location_l2[3]);
