...
module EQUALIZER_engine #(
  ...
  bit    [N_FFT -1 : 0]   [31:0]              out32                    ;  
  logic  [N_FFT -1 : 0]                       valid_in                 ;
  logic                                       valid_out                ;

  logic  [N_FFT -1 : 0]   [DATA_WIDTH-1:0]    in_r                     ;
  logic  [N_FFT -1 : 0]   [DATA_WIDTH-1:0]    in_i                     ;
  logic  [N_FFT -1 : 0]   [DATA_WIDTH-1:0]    out_r                    ;
  logic  [N_FFT -1 : 0]   [DATA_WIDTH-1:0]    out_i                    ;
...
generate
  ...
  begin
    assign in_r [i]                           = in32[i] [DATA_WIDTH-1    : 0  ] ;
    assign in_i [i]                           = in32[i] [DATA_WIDTH+16-1 : 16 ] ;

    assign out32[i] [DATA_WIDTH   -1 : 0  ]   = out_r[i]                        ;
    assign out32[i] [DATA_WIDTH+16-1 : 16 ]   = out_i[i]                        ;
  end
endgenerate
