...
(
  ...
  localparam AXI_CLUSTER_SOC_ID_WIDTH   = AXI_SOC_CLUSTER_ID_WIDTH + $clog2(`NB_SLAVE);
   // localparam AXI_CLUSTER_SOC_ID_WIDTH   = 12;
...
//********************************************************************************
  //********** SOC TO CLUSTER DOMAINS SIGNALS Modify by Ali Mohamed*****************
  //********************************************************************************

  // Per-cluster control and status — one entry per cluster
  logic [`NB_CLUSTERS-1:0]        s_cluster_clk;
  logic [`NB_CLUSTERS-1:0]        s_cluster_rstn;
  logic [`NB_CLUSTERS-1:0]        s_cluster_busy;
  logic [`NB_CLUSTERS-1:0]        s_cluster_irq;   // single: FC-level interrupt, not per-cluster
  logic                           s_cluster_rtc;   // single: real-time clock, shared
  logic [`NB_CLUSTERS-1:0]        s_cluster_fetch_enable;
  logic [`NB_CLUSTERS-1:0][63:0]  s_cluster_boot_addr;
  logic                           s_cluster_test_en;  // single: test mode is global
  logic                           s_cluster_pow;      // single: power control is global
  logic                           s_cluster_byp;      // single: bypass is global

  logic [`NB_CLUSTERS-1:0]        s_dma_pe_evt_ack;
  logic [`NB_CLUSTERS-1:0]        s_dma_pe_evt_valid;
  logic [`NB_CLUSTERS-1:0]        s_dma_pe_irq_ack;
  logic [`NB_CLUSTERS-1:0]        s_dma_pe_irq_valid;
  logic [`NB_CLUSTERS-1:0]        s_pf_evt_ack;
  logic [`NB_CLUSTERS-1:0]        s_pf_evt_valid;

  // Per-cluster event buses
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                        s_event_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                        s_event_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][EVENT_WIDTH-1:0]  s_event_dataasync;

  // Per-cluster C2S AXI async signals (Cluster → SoC)
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_aw_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_aw_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_CLUSTER_SOC_AW_WIDTH-1:0] s_cluster_soc_bus_aw_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_ar_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_ar_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_CLUSTER_SOC_AR_WIDTH-1:0] s_cluster_soc_bus_ar_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_w_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_w_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_CLUSTER_SOC_W_WIDTH-1:0]  s_cluster_soc_bus_w_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_r_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_r_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_CLUSTER_SOC_R_WIDTH-1:0]  s_cluster_soc_bus_r_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_b_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_cluster_soc_bus_b_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_CLUSTER_SOC_B_WIDTH-1:0]  s_cluster_soc_bus_b_data;

  // Per-cluster S2C AXI async signals (SoC → Cluster)
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_aw_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_aw_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_SOC_CLUSTER_AW_WIDTH-1:0] s_soc_cluster_bus_aw_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_ar_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_ar_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_SOC_CLUSTER_AR_WIDTH-1:0] s_soc_cluster_bus_ar_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_w_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_w_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_SOC_CLUSTER_W_WIDTH-1:0]  s_soc_cluster_bus_w_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_r_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_r_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_SOC_CLUSTER_R_WIDTH-1:0]  s_soc_cluster_bus_r_data;

  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_b_wptr;
  logic [`NB_CLUSTERS-1:0][LOG_DEPTH:0]                                    s_soc_cluster_bus_b_rptr;
  logic [`NB_CLUSTERS-1:0][2**LOG_DEPTH-1:0][AXI_SOC_CLUSTER_B_WIDTH-1:0]  s_soc_cluster_bus_b_data;
 ...
 //********************************************************************************
  //********** CLUSTER DOMAINS — one per cluster Modify by Ali Mohamed**************
  //********************************************************************************

genvar cl;
  generate
  	for (cl = 0; cl < `NB_CLUSTERS; cl++) begin : gen_clusters
  		cluster_domain#(
		  //CLUSTER PARAMETERS
			.CORE_TYPE_CL        ( CORE_TYPE_CL    ),
			.USE_HWPE_CL         ( USE_HWPE_CL     ),
			.CLUSTER_ID          ( cl              ),  // NEW: unique cluster ID
			.NB_CORES            (`NB_CORES        ),
			.NB_HWPE_PORTS       (        8        ),
			.NB_DMAS             (        4	       ),
			.TCDM_SIZE           (      64*1024	   ),
		 ...
		 // AXI SOC TO CLUSTER
		  ...
		  // PERIPH PARAMETERS
		    ...
		    (
			.clk_i                        ( s_cluster_clk[cl]                ),
			.rst_ni                       ( s_cluster_rstn[cl]               ),
			.ref_clk_i                    ( s_ref_clk                        ),

			.async_cluster_events_wptr_i  ( s_event_wptr[cl]                 ),
			.async_cluster_events_rptr_o  ( s_event_rptr[cl]                 ),
			.async_cluster_events_data_i  ( s_event_dataasync[cl]            ),

			.dma_pe_evt_ack_i             ( s_dma_pe_evt_ack[cl]             ),
			.dma_pe_evt_valid_o           ( s_dma_pe_evt_valid[cl]           ),
			.dma_pe_irq_ack_i             ( s_dma_pe_irq_ack [cl]            ),
			.dma_pe_irq_valid_o           ( s_dma_pe_irq_valid               ),

			.dbg_irq_valid_i              ( s_dbg_irq_valid                  ),


			.pf_evt_ack_i                 ( s_pf_evt_ack[cl]                 ),
			.pf_evt_valid_o               ( s_pf_evt_valid[cl]               ),
			
			.busy_o                       ( s_cluster_busy[cl]               ),

			.async_data_master_aw_wptr_o  ( s_cluster_soc_bus_aw_wptr[cl]    ),
			.async_data_master_aw_rptr_i  ( s_cluster_soc_bus_aw_rptr[cl]    ),
			.async_data_master_aw_data_o  ( s_cluster_soc_bus_aw_data[cl]    ),
			
			.async_data_master_ar_wptr_o  ( s_cluster_soc_bus_ar_wptr[cl]    ),
			.async_data_master_ar_rptr_i  ( s_cluster_soc_bus_ar_rptr[cl]    ),
			.async_data_master_ar_data_o  ( s_cluster_soc_bus_ar_data[cl]    ),
			
			.async_data_master_w_wptr_o   ( s_cluster_soc_bus_w_wptr[cl]     ),
			.async_data_master_w_rptr_i   ( s_cluster_soc_bus_w_rptr[cl]     ),
			.async_data_master_w_data_o   ( s_cluster_soc_bus_w_data[cl]     ),
				                                                       
			.async_data_master_r_wptr_i   ( s_cluster_soc_bus_r_wptr[cl]     ),
			.async_data_master_r_rptr_o   ( s_cluster_soc_bus_r_rptr[cl]     ),
			.async_data_master_r_data_i   ( s_cluster_soc_bus_r_data[cl]     ),
				                                                       
			.async_data_master_b_wptr_i   ( s_cluster_soc_bus_b_wptr[cl]     ),
			.async_data_master_b_rptr_o   ( s_cluster_soc_bus_b_rptr[cl]     ),
			.async_data_master_b_data_i   ( s_cluster_soc_bus_b_data[cl]     ),


			.async_data_slave_aw_wptr_i  ( s_soc_cluster_bus_aw_wptr[cl]    ),
			.async_data_slave_aw_rptr_o  ( s_soc_cluster_bus_aw_rptr[cl]    ),
			.async_data_slave_aw_data_i  ( s_soc_cluster_bus_aw_data[cl]    ),
			
			.async_data_slave_ar_wptr_i  ( s_soc_cluster_bus_ar_wptr[cl]    ),
			.async_data_slave_ar_rptr_o  ( s_soc_cluster_bus_ar_rptr[cl]    ),
			.async_data_slave_ar_data_i  ( s_soc_cluster_bus_ar_data[cl]    ),
			
			.async_data_slave_w_wptr_i   ( s_soc_cluster_bus_w_wptr[cl]     ),
			.async_data_slave_w_rptr_o   ( s_soc_cluster_bus_w_rptr[cl]     ),
			.async_data_slave_w_data_i   ( s_soc_cluster_bus_w_data[cl]     ),
				                                                       
			.async_data_slave_r_wptr_o   ( s_soc_cluster_bus_r_wptr[cl]     ),
			.async_data_slave_r_rptr_i   ( s_soc_cluster_bus_r_rptr[cl]     ),
			.async_data_slave_r_data_o   ( s_soc_cluster_bus_r_data[cl]     ),
				                                                       
			.async_data_slave_b_wptr_o   ( s_soc_cluster_bus_b_wptr[cl]     ),
			.async_data_slave_b_rptr_i   ( s_soc_cluster_bus_b_rptr[cl]     ),
			.async_data_slave_b_data_o   ( s_soc_cluster_bus_b_data[cl]     )
		    );
  	end
  endgenerate
